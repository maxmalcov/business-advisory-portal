    ...
    <Card className="overflow-hidden border border-border shadow-sm">
      ...
      <CardContent className="pt-4 grid grid-cols-1 md:grid-cols-2 gap-4 bg-card">
        <div>
          ...
          <p className="text-sm font-semibold text-foreground">{employee.dniTie || '-'}</p>
        </div>
        
        <div>
          <p className="text-sm font-medium text-muted-foreground">{t('hr.index.employee.details.id-document')}</p>
          {employee.idDocument ? (
            <DocumentDisplay documentUrl={employee.idDocument} />
          ) : (
            <p className="text-sm text-muted-foreground">{t('hr.index.employee.details.no-document')}</p>
          )}
        </div>
      </CardContent>
    </Card>
