...
const ServiceRequests: React.FC = () => {
  ...
  const getStatusBadgeProps = (status: string) => {
    switch (status.toLowerCase()) {
      case 'pending':
        return { variant: 'outline' as const, children: <><Clock className="mr-1 h-3 w-3" /> Pending</> };
      case 'in progress':
        return { variant: 'secondary' as const, children: <><Loader2 className="mr-1 h-3 w-3 animate-spin" /> In Progress</> };
      case 'completed':
        return { variant: 'default' as const, children: <><CheckCircle className="mr-1 h-3 w-3" /> Completed</> };
      default:
