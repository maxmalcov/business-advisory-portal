      ...
      try {
        ...
        if (invoices) {
          ...
          const salesInvoices = invoices.filter(
            (inv) => inv.invoice_type === 'sale',
          );
          const supplierInvoices = invoices.filter(
            (inv) => inv.invoice_type === 'supplier',
          );
          ...
          const lastMonthInvoices = invoices.filter((inv) => {
            ...
            return (
              ...
              (lastMonth === 11
                ? invDate.getFullYear() === currentYear - 1
                : invDate.getFullYear() === currentYear)
            );
          });
          ...
          if (invoices.length > 0) {
            console.log(
              'Processing invoices for monthly chart:',
              invoices.length,
            );
            ...
            console.log(
              'No invoices found to process for monthly chart, using mock data',
            );
          ...
          console.log(
            'No invoices data returned from Supabase, using mock data',
          );
        ...
        setError(
          err instanceof Error
            ? err
            : new Error('Unknown error fetching invoice stats'),
        );
        ...
        toast({
          ...
          description:
            'There was a problem loading your invoice data. Using mock data instead.',
        });
      } finally {
        setLoading(false);
      }
