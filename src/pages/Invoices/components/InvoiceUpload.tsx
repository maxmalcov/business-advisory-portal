...
const InvoiceUpload: React.FC = () => {
  ...
  const handleSendEmail = async () => {
    ...
    try {
      sendEmail(user.outgoingInvoiceEmail, 'New invoice', `New sale invoice was uploaded by ${user.name}`)

      log({ action: 'Invoice uploaded', description: `Sale invoice uploaded by ${user.name}`, user: user.email, level: 'info', category: 'invoice'})
    } catch (error) {
  ...
  return <Card className="transition-all duration-200 hover:shadow-md border-primary/10">
      ...
      <CardContent className="space-y-8">
        ...
        <div className="bg-muted/50 rounded-lg p-6">
          <UploadGuidelines emailAddress={user?.outgoingInvoiceEmail} emailType="outgoing" />
        </div>
        
        {/* File upload section with enhanced styling */}
        <FileUploadSection selectedFiles={selectedFiles} uploadedFiles={uploadedFiles} isDragging={isDragging} isLoading={isLoading} isSending={isSending} uploadProgress={uploadProgress} uploadComplete={uploadComplete} uploadSuccess={uploadSuccess} uploadError={uploadError} onDragOver={handleDragOverAppend} onDragLeave={handleDragLeave} onDrop={handleDropAppend} onFileSelect={handleFileSelect} onRemoveFile={handleRemoveFile} onSendEmail={handleSendEmail} onResetUpload={handleResetUpload} onAddMoreFiles={handleAddMoreFiles} onFileChange={handleFileChangeWithMode} remainingFilesCount={remainingFilesCount} hasReachedFileLimit={fileLimit} />
      </CardContent>
    </Card>;
};
