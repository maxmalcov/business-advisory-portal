...
export function useInvoiceFilter(invoices: InvoiceUpload[], initialFilterType: FilterType = 'all') {
  const [searchQuery, setSearchQuery] = useState('');
  const [filterType, setFilterType] = useState<FilterType>(initialFilterType);
  const [filteredInvoices, setFilteredInvoices] = useState<InvoiceUpload[]>(invoices);
