...
import {
  subscriptionTypeFormSchema,
  SubscriptionTypeFormValues,
} from './subscription-type/schema';
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>
            {editSubscription
              ? t('subscriptions.admin.edit-type')
              : t('subscriptions.admin.add-new-type')}
          </DialogTitle>
        </DialogHeader>
        ...
        <Form {...form}>
          <form
            onSubmit={form.handleSubmit(
              editSubscription ? handleEditSubmit : handleSubmit,
            )}
            className="space-y-4"
          >
            ...
            <DialogFooter className="mt-6">
              ...
              <Button type="submit" disabled={isSubmitting}>
                {editSubscription
                  ? t('subscriptions.admin.edit-button')
                  : isSubmitting
                    ? t('subscriptions.admin.creating')
                    : t('subscriptions.admin.creating.type')}
              </Button>
            </DialogFooter>
          </form>
        </Form>
