...
const Subscriptions: React.FC = () => {
  ...
  const [selectedTool, setSelectedTool] = useState<SubscriptionTool | null>(
    null,
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      const { data: typesData, error: typesError } =
        await subscriptionTypeTable().select('*');
      if (!typesError && typesData) {
        setSubscriptionTypes(typesData as any);
      }

      const { data: requestData, error: requestError } =
        await subscriptionRequestsTable().select('*').eq('user_id', user.id);
      if (requestError) {
  ...
  const handleRequestAccess = async () => {
    ...
    try {
      ...
      const { error: notificationError } = await supabase.functions.invoke(
        'notify-admin-subscription-request',
        {
          body: {
      ...
      if (selectedTool.status == 'inactive') {
        ...
        if (dbError) {
          ...
          const { error } = await subscriptionRequestsTable()
              .update({
                status: 'pending',
              })
              .eq('user_id', user.id)
        ...
        const { error: dbError } = await subscriptionRequestsTable()
          .update({
            status: 'pending',
          })
          .eq('user_id', user.id)
      ...
      sendEmail(
        ...
        `A client has submitted a new subscription request.
...
You can manage this request in the admin dashboard.`,
      ...
      log({
        ...
        description: `New subscriptoin request ${selectedTool.name} from ${user?.name || user?.email || 'Unknown User'}`,
